...
spc_detected_APC <- dplyr::filter(soilseq_plate_counts_reorganized, Count.type=="Aerobic plate counts", present=="detected")
spc_detected_TC <- dplyr::filter(soilseq_plate_counts_reorganized, Count.type=="Total coliforms", present=="detected")
spc_detected_APC_and_TC <- dplyr::filter(soilseq_plate_counts_boxplot, Count.type=="Aerobic plate counts" | Count.type=="Total coliforms", present=="detected")
```
...
#Shapiro-Wilk test for normality (p > 0.05, normally distributed)
  ...
  shapiro.test(tc_bootie_swab$with.LOD.2) #not normally distributed
  qqnorm(tc_bootie_swab$with.LOD.2);qqline(tc_bootie_swab$with.LOD.2, col = 2)
  plot(density(tc_bootie_swab$with.LOD.2,na.rm=TRUE))
